    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
      ...
      {reports.length > 0 ? (
                      ...
                      <td style={{ padding: '1.25rem 2rem' }}>
                         {report.csatScore ? (
                           <div style={{ display: 'flex', gap: '3px' }}>
                             {[...Array(5)].map((_, i) => (
                               <div key={i} style={{ width: '4px', height: '14px', background: i < report.csatScore ? 'var(--primary)' : 'rgba(255,255,255,0.05)', borderRadius: '1.5px' }}></div>
                             ))}
                           </div>
                         ) : (
                           <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>Unrated</span>
                         )}
                      </td>
                      <td style={{ padding: '1.25rem 2rem' }}>
        ...
        <PremiumEmptyState 
          ...
          onAction={() => {}} // This should open the generator selector if integrated
        />
      )}
    </div>
  );
